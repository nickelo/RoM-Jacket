...
:injecAMIGTVs
if "%AMIGTVHDD%"=="" "%GBC%\fart.exe" "Config.fs-uae" [HDD] --remove
if "%AMIGTVDSK%" NEQ "" "%GBC%\fart.exe" "Config.fs-uae"  [CDROM] "%AMIGTVDSK%"
if "%AMIGTVHDD%" NEQ "" "%GBC%\fart.exe" "Config.fs-uae" [HDD] "%AMIGTVHDD%"
exit /b

:AMIGTVend
"%GBC%\fart.exe" "Config.fs-uae" [ROMPTH] "%GBG%\%GAM%\%CONS%\%AMIGTV%\%CSTM%"
