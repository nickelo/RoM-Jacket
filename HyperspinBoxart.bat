...
"%GBC%\wbox.exe" "RJ_GUI" "Would you like to enable fuzzy matching for your %RJCP% artwork migration?" "Yes;No" /DB=2
...
:realmatch
for /f "delims=" %%a in ('dir /b/a-d "%BARTLOC%\*.png"') do (
set HSNM=%%~na
CALL :hsna
)
goto :complete
...
for %%a in ("%BARTLOC%\%HSNM%.png") do copy /Y "%%~a" "%RJNM%\Folder.png"
exit /b 

:fuzmtch
for /f "tokens=1 delims=([" %%a in ('dir /b/a-d "%BARTLOC%\*.png"') do (
set HSNM=%%~na
CALL :hsnn
)
...
:hsnn
pushd "%GBG%\%GAM%\%CONS%\%RJCP%"
for /f "delims=" %%a in ('dir /b /ad "%HSNM%*"') do (
set RJNM=%%~dpnxa
set DRJNM=%%~nxa
CALL :newnames
popd
...
:newnames
if "%HSNM%" NEQ "%DRJNM%" exit /b
for %%a in ("%BARTLOC%\%HSNM%*") do copy /Y "%%~a" "%RJNM%\Folder.png"
exit /b
