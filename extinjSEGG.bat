...
for /f "delims=" %%i in ('dir /b /a-d "%GBE%\%EMUZ%\%FUSN%\32X_G*"') do SET SEG32XG=%%s do SET SEG32XG=%%s
pushd "%GBC%\net\%LNCH%\%SEGG%"
...
if "%RECONF%"=="1" %COPYTYPE% "%GBC%\net\%LNCH%\%SEGG%\%FUSN%.ini" "%%~g\%FUSN%.ini"
set CSTM=%%~g
