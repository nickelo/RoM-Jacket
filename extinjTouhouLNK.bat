...
"%GBC%\fart.exe" "*.bat" "[LNK]" "REG ADD \"HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers\" /f /v \"%target%\" /t REG_SZ /d \"WINXPSP3 RUNASADMIN\""
"%GBC%\fart.exe" "*.bat" "REM[SDK]" "[SDK]"
"%GBC%\fart.exe" "*.bat" "[SDK]" "start /w \"\" \"%GBE%\%EMUZ%\%BATCHL%\" \"%GAMNAM%.lnk\""
popd