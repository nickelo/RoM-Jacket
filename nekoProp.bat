...
if %ERRORLEVEL%==1 goto :PC98JOY
if %ERRORLEVEL%==2 goto :PC98OTH
goto :QUITOUT
:PC98JOY
for /f "delims=" %%a in ('%GBC%\Wfile.exe "SET SOURCECFG=" "%GBG%\%GAM%\%CONS%\%ZXSP%\np21nt.ini" "Select Configured np21nt.ini" /noquote') do %%a
if %errorlevel%==0 exit /b
%BSTRT% "%WBUSY%" "RJ_GUI" "Propagating NEC PC98 Joystick settings" /marquee
for /f  "delims=" %%a in ('dir /b/a-d/s "%GBG%\%GAM%\%CONS%\%ZXSP%\%GAMEDIR%*np21nt.ini"') do (
set NEWCONF=%%~a
...
for /f "delims=" %%a in ('%GBC%\Wfile.exe "SET SOURCECFG=" "%GBG%\%GAM%\%CONS%\%ZXSP%\np21nt.ini" "Select Configured np21nt.ini" /noquote') do %%a
if %errorlevel%==0 exit /b
%BSTRT% "%WBUSY%" "RJ_GUI" "Propagating NEC PC98 settings" /marquee
for /f  "delims=" %%a in ('dir /b/a-d/s "%GBG%\%GAM%\%CONS%\%ZXSP%\%GAMEDIR%*np21nt.ini"') do (
set NEWCONF=%%~a
