if "%TMPDIR%" EQU "%GBG%\%ROMJ%" goto :samef
if "%TMPDIR%" EQU "" goto :samef

:setchkf
for /f "delims=" %%i in ('dir /b /a-d-h "%GBG%\%ROMJ%\%TMPDIR%"') do (
SET CHKFILE=
set FZEXTCHK=%%~xi
call :CHKSUMf
...
if /i "%FZEXTCHK%" EQU ".sg" SET CHKFILE=%SEGMS%
...
if /i "%FZEXTCHK%" EQU ".rom" SET CHKFILE=%COLEC%
...
if /i "%FZEXTCHK%" EQU ".vb" SET CHKFILE=%NVB%
...
if /i "%FZEXTCHK%" EQU ".atz" SET CHKFILE=%AT8H%
...
if /i "%FZEXTCHK%" EQU ".2hd" SET CHKFILE=%X68K%
if /i "%FZEXTCHK%" EQU ".2dd" SET CHKFILE=%X68K%
if /i "%FZEXTCHK%" EQU ".dup" SET CHKFILE=%X68K%
...
if /i "%FZEXTCHK%" EQU ".dup" SET CHKFILE=%PC98%
if /i "%FZEXTCHK%" EQU ".2hd" SET CHKFILE=%PC98%
if /i "%FZEXTCHK%" EQU ".thd" SET CHKFILE=%PC98%
if /i "%FZEXTCHK%" EQU ".nhd" SET CHKFILE=%PC98%     
exit /b

:renamef
if not exist "%GBG%\%GAM%\%CONS%\%CHKFILE%" mkdir "%GBG%\%GAM%\%CONS%\%CHKFILE%"
move /Y "%GBG%\%ROMJ%\%TMPDIR%\*%FZEXTCHK%" "%GBG%\%GAM%\%CONS%\%CHKFILE%"
:samef