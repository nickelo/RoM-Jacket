...
:AUTOINSTALL
call :wget_%CEMU%
if "%EMUNOTHERE%" NEQ "1" %WFINS% "RJ_GUI" "%CEMU% Installed." /Stop /sound /timeout:1
exit /b

:RECHECK
%WFINS% "RJ_GUI" "Complete." /Stop /sound /timeout:1
...
:NOEMU
%WBUSY% "RJ_GUI" "%MISAUTO% Not Found." /Stop /sound /timeout:1
...
:INSTCOMPLETE
if not exist "%GBE%\%EMUZ%\%BATCHL%\%BATCHL%.exe" "%SEVENZIP%" e -y "%GBC%\net\%EMEX%\%EMEX%.7z" -o"%GBE%\%EMUZ%\%BATCHL%" "%BATCHL%\%BATCHL%.exe"
%WFINS% "RJ_GUI" "Complete." /Stop  /timeout:1
